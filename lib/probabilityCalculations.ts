...
import {userQuestion, databaseQuestion, tableQuestion} from "./types.ts"
import { addQuestionToDatabase, getMarketBySlug, getQuestionsFromDatabase, placeBetBySlug } from '@/lib/api';
...
export const betROIOverTime = (betROI, currentTime, marketCorrectionTime) => {
...
const processData = async (userData: userQuestion): Promise<databaseQuestion> => {
    ...
    const response = await getMarketBySlug();
    const marketProbability = parseFloat(response.probability);
    const thingToBuy = buyYes(response.probability, userProbability);
    const marketWinChance = marketWinChance(response.probability, thingToBuy);
    const myWinChance = myWinChance(userProbability, thingToBuy);
    const marketReturn = marketReturn(marketWinChance);
    const kellyBetProportion = kellyBetProportion(marketReturn, myWinChance);
    const betEVreturn = betEVreturn(marketWinChance, myWinChance);
    const betROI = betROI(betEVreturn, marketWinChance);
    const betROIOverTime = betROIOverTime(betROI, currentTime, market)
...
const processNewAndUpdatedData = async (modifiedData, unmodifiedData) => {
    ...
    setDatbaseQuestionData(sortedData);
